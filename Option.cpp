 ...
 *
 * Returns a vector of pairs where each pair consists of a spot price and the corresponding option price.
 *
 * @return Vector of (spot price, option price) pairs.
 */
std::vector<std::pair<double, double>> Option::price_plot() {
    ...
    std::vector<std::pair<double, double>> graph(spot_mesh_ - 1);
    double S = dS;

    for (size_t ii = 1; ii < spot_mesh_; ii++) {
        graph[ii - 1] = std::make_pair(S, grid[ii][0]);
        S += dS;
    }
...
/**
 * @brief Generates a plot of the delta values for a range of spot prices.
 *
 * This function computes the delta for a series of spot prices within a specified range
 * and returns a vector of pairs. Each pair contains a spot price and the corresponding delta value.
 * The step size for incrementing the spot price is calculated as a fraction of the minimum spot price.
 *
 * @param h A small parameter affecting the computation of delta.
 * @return A vector of pairs, where each pair contains:
 *         - The spot price as the first element.
 *         - The corresponding delta value as the second element.
 */
std::vector<std::pair<double, double>> Option::delta_plot(double h) {
    double S_min = S0_, S_max = 4 * S0_;
    double fraz = 0.1 * S_min;
    size_t num_points = static_cast<size_t>((S_max - S_min) / fraz) + 1;
    std::vector<std::pair<double, double>> graph(num_points);
    double S = S_min;

    for (size_t cont = 0; cont < num_points; cont++) {
        S0_ = S;
        graph[cont] = std::make_pair(S, delta(S * h));
        S += fraz;
    }
 ...
 *
 * Determines the boundary separating the exercise and continuation regions for an American option.
 *
 * @return Vector of (time, exercise boundary spot price) pairs.
 */
std::vector<std::pair<double, double>> Option::exercise_boundary() {
    ...
    for (size_t ii = 0; ii < time_mesh_ - 1; ii++) {
        ...
        for (size_t jj = 1; jj < spot_mesh_; jj++) {
            dist = abs(grid[jj][ii] - contract_type_ * (dS * jj - K_));
            if (dist < min) {
                min = dist;
                S = dS * jj;
            }
        }
        graph.push_back(std::make_pair(ii * dT, S));
    }
